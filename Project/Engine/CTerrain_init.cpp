...
void CTerrain::LoadTileTex()
{
	...
	if(nullptr==CResMgr::GetInst()->FindRes<CTexture>(L"TileArrTex"))
	m_pTileTex = CResMgr::GetInst()->Load<CTexture>(L"TileArrTex", L"texture\\tile\\TILE_ARR.dds");
	if (nullptr == CResMgr::GetInst()->FindRes<CTexture>(L"TileNArrTex"))
	m_pTileNTex = CResMgr::GetInst()->Load<CTexture>(L"TileNArrTex", L"texture\\tile\\TILE_ARR_N.dds");
	if (nullptr == CResMgr::GetInst()->FindRes<CTexture>(L"BrushArrTex"))
...
void CTerrain::CreateTerrainMesh()
{
	...
	if (nullptr == m_pMesh)
	{
		if (nullptr == CResMgr::GetInst()->FindRes<CMesh>(L"TerrainMesh"))
		{
			m_pMesh = new CMesh;
		...
		{
			m_pMesh = CResMgr::GetInst()->FindRes<CMesh>(L"TerrainMesh");
		}
	}
	else
	{
