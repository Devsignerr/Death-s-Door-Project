...
#define ChangeFrame   g_int_3
#define LerpTime      g_float_1
// ===========================
[numthreads(256, 1, 1)]
void CS_Animation3D(int3 _iThreadIdx : SV_DispatchThreadID)
{
    ...
    uint iChangeFrameDataIdx = BoneCount * ChangeFrame  + _iThreadIdx.x;
    
    float4 vScale = float4(0.f, 0.f, 0.f, 0.f);

    float4 vTrans = float4(0.f, 0.f, 0.f, 0.f);

    float4 qRot = float4(0.f, 0.f, 0.f, 0.f);
    
    if (ChangeFrame>0)
    {
        vScale = lerp(g_arrFrameTrans[iFrameDataIndex].vScale, g_arrFrameTrans[iChangeFrameDataIdx].vScale, Ratio);
        vTrans = lerp(g_arrFrameTrans[iFrameDataIndex].vTranslate, g_arrFrameTrans[iChangeFrameDataIdx].vTranslate, Ratio);
        qRot = QuternionLerp(g_arrFrameTrans[iFrameDataIndex].qRot, g_arrFrameTrans[iChangeFrameDataIdx].qRot, Ratio);
    }
    else
    {
