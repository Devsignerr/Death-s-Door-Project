...
void CSkullScript::awake()
{
    ...
    CreateCol(L"SkullAttackCol", Vec3(0.0f, 400.0f, 250.0f), Vec3(250.0f, 100.0f, 500.0f), LAYER_TYPE::MONSTER_ATTACK_COL);
...
{
    if (true == m_MeleeAttackCheck)
    {
        m_MeleeAttackDelay += fDT;

        if (0.8f < m_MeleeAttackDelay)
        {
            m_MeleeAttackDelay = 0.0f;
            ...
            if (RangeSearch(m_MeleeAttackRange))
            {
                ...
                ChangeState(MONSTERSTATE::ATTACK, 0.1f, L"Melee");
            }
            else if (false == RangeSearch(m_MeleeAttackRange) && true == RangeSearch(m_LongDistanceAttackRange))
            {
            ...
            {
                ChangeState(MONSTERSTATE::ATTACK, 0.1f, L"LongDistance");
            }
